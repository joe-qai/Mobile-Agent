...
APP_PACKAGES: dict[str, str] = {
    ...
    "u2": "com.github.uiautomator",
    "lockin smart": "com.lockin.smart"
}
...
_EXTRA_MAPPINGS_PATH = os.path.join(os.path.dirname(__file__), "extra_mappings.json")


def _load_extra_mappings():
    try:
        with open(_EXTRA_MAPPINGS_PATH, encoding="utf-8") as f:
            extra = json.load(f)
            APP_PACKAGES.update(extra)
    except (FileNotFoundError, json.JSONDecodeError):
        pass


def _save_extra_mappings():
    with open(_EXTRA_MAPPINGS_PATH, "w", encoding="utf-8") as f:
        json.dump(APP_PACKAGES, f, ensure_ascii=False, indent=2)


def add_package_mapping(app_name: str, package_name: str):
    """添加或更新应用-包名映射，持久化到 JSON"""
    APP_PACKAGES[app_name] = package_name
    _save_extra_mappings()


_load_extra_mappings()
